...
parser.add_argument(
    "-c", "--media-comment", help="comment for media element", metavar="text"
)
parser.add_argument(
    "-l", "--media-link", help="link for media element", metavar="url"
)
parser.add_argument(
    "-f",
    "--media-file",
    type=existing_file,
    help="file for media element",
    metavar="file",
)
parser.add_argument(
    "-e",
...
group.add_argument(
    ...
    help="timestamp of referenced note",
    metavar="timestamp",
)
group.add_argument(
    "-u", "--reply-url", help="link for referenced URL", metavar="url"
)
parser.add_argument(
    "-U", "--reply-title", help="title for reply", metavar="text"
)
group.add_argument("-r", "--reply-to", help="user for reply", metavar="user")
parser.add_argument(
    "-t", "--reply-quote", help="referenced note’s content", metavar="text"
)
parser.add_argument(
    "-i",
    "--reply-time",
...
parser.add_argument(
    ...
    help="importance advice",
    metavar="importance",
)
parser.add_argument(
    "-g",
    "--urgency",
    choices=["whenever", "soon", "now"],
    help="urgency advice",
    metavar="urgency",
)
parser.add_argument("text", help="content of note to post")
args = parser.parse_args()
if any([args.media_comment, args.media_link, args.media_file]) and not all([
    args.media_comment,
    args.media_link,
    args.media_file,
]):
...
if args.reference_url:
    note["references"] = [
        {"title": t, "url": u}
        for t, u in zip(args.reference_title, args.reference_url)
    ]
if args.reply_self:
    note["in_reply_url"] = {
    ...
    note["in_reply_url"] = {"title": args.reply_title, "url": args.reply_url}
elif args.reply_to:
    note["in_reply_to"] = {
